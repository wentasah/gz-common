...
int Image::Load(const std::string &_filename)
{
  ...
  if (exists(this->dataPtr->fullName))
  {
    ...
    if (bitmap == NULL)
    {
      gzerr << "Failed to load file [" << this->dataPtr->fullName << "]: " << stbi_failure_reason() << std::endl;
      return -1;
    }
...
void Image::SavePNG(const std::string &_filename)
{
  if (this->dataPtr->bits_per_channel != 8) {
    gzerr << "Cannot write " << this->dataPtr->bits_per_channel << "-bit PNG image (" << _filename << ")\n";
    return;
  }
  int ret = stbi_write_png(_filename.c_str(), dataPtr->width, dataPtr->height,
                           dataPtr->channels, dataPtr->bitmap, dataPtr->width * dataPtr->channels);
  if (ret == 0)
...
{
  if (this->dataPtr->bits_per_channel != 8) {
    gzerr << "Cannot export " << this->dataPtr->bits_per_channel << "-bit PNG image\n";
    return;
  }
  int ret = stbi_write_png_to_func(vector_write_func, &buffer, dataPtr->width,
                                   dataPtr->height, dataPtr->channels, dataPtr->bitmap,
    ...
    Image::PixelFormatType _format)
{
  ...
  gzwarn << "Image::SetFromData " << _width << "x" << _height << "\n";
  unsigned int bpp;
  int scanlineBytes;
                                  ...
                                  Image::PixelFormatType _format)
{
  if (this->dataPtr->bitmap)
    stbi_image_free(this->dataPtr->bitmap);
  this->dataPtr->bitmap = NULL;
  gzwarn << "Image::SetFromCompressedData size:" << _size << " format:" << _format << "\n";
...
{
  return this->dataPtr->width * this->dataPtr->channels * this->dataPtr->bits_per_channel / 8;
}
//////////////////////////////////////////////////
std::vector<unsigned char>
Image::Implementation::DataWithChannels(int out_channels) const {
  std::vector<unsigned char> data;
  const size_t size = this->width * this->height * this->channels * this->bits_per_channel / 8;
  ...
  if (this->channels != out_channels) {
    ...
    case 8:
      bitmap_rgb = stbi__convert_format(bitmap_copy, this->channels, out_channels, this->width, this->height);
      break;
    case 16:
      bitmap_rgb = (unsigned char *)stbi__convert_format16((uint16_t*)bitmap_copy, this->channels, out_channels, this->width, this->height);
      break;
    ...
    data = this->DataImpl(bitmap_rgb, this->width * this->height * out_channels * this->bits_per_channel / 8);
...
//////////////////////////////////////////////////
std::vector<unsigned char> Image::Implementation::DataImpl(void *_img, size_t size) const
{
...
math::Color Image::Pixel(unsigned int _x, unsigned int _y) const
{
  ...
  case 32:
    ...
    break;
  default:
    gzerr << "Image: Unsupported bits per channel [" << this->dataPtr->bits_per_channel << "] \n";
    return clr;
  }
...
//////////////////////////////////////////////////
constexpr static float sinc(float value)
{
  if (value != 0) {
    value *= M_PI;
...
constexpr static float lanczos3(float x)
{
  x = fabsf(x);
  if (x < 3.0f) {
...
static void applyLanczos3(void *_dst, int dst_w, int dst_h, void *_src,
                          int src_w, int src_h) {
  ...
  for (int y_dst = 0; y_dst < dst_h; y_dst++) {
    for (int x_dst = 0; x_dst < dst_w; x_dst++) {
      const float x = (float)x_dst * src_w / dst_w;
      const float y = (float)y_dst * src_h / dst_h;
      float acc[CH] = {0.0};
      for (int iy = -2; iy <= 3; iy++) {
        for (int ix = -2; ix <= 3; ix++) {
          int x_src = (int)x + ix;
          int y_src = (int)y + iy;
          if (x_src >= 0 && x_src < src_w && y_src >= 0 && y_src < src_h) {
            float mul = lanczos3(x - x_src) * lanczos3(y - y_src);
            for (unsigned ch = 0; ch < CH; ch++) {
              acc[ch] += mul * src[y_src * src_w * CH + x_src * CH + ch];
            }
          }
        }
      }
      for (unsigned ch = 0; ch < CH; ch++) {
        dst[y_dst * dst_w * CH + x_dst * CH + ch] = std::clamp(0, (int)acc[ch], 255);
      }
    }
  }
}
...
template <typename T>
static bool applyMultichannelLanczos3(void *_dst, int dst_w, int dst_h, void *_src, int src_w, int src_h, unsigned channels) {
  switch (channels) {
  case 1:
    applyLanczos3<T, 1>(_dst, dst_w, dst_h, _src, src_w, src_h);
    break;
  case 2:
    applyLanczos3<T, 2>(_dst, dst_w, dst_h, _src, src_w, src_h);
    break;
  case 3:
    applyLanczos3<T, 3>(_dst, dst_w, dst_h, _src, src_w, src_h);
    break;
  case 4:
    applyLanczos3<T, 4>(_dst, dst_w, dst_h, _src, src_w, src_h);
    break;
  default:
    gzerr << "Image: Unsupported number of channels [" << channels << "] \n";
    return false;
...
{
  void *src = this->dataPtr->bitmap;
  void *dst = STBI_MALLOC(_width * _height * this->dataPtr->channels * this->dataPtr->bits_per_channel / 8);
  int w = this->dataPtr->width;
  int h = this->dataPtr->height;
  ...
  switch (this->dataPtr->bits_per_channel) {
  case 8:
    ok = applyMultichannelLanczos3<uint8_t>(dst, _width, _height, src, w, h, this->dataPtr->channels);
    break;
  case 16:
    ok = applyMultichannelLanczos3<uint16_t>(dst, _width, _height, src, w, h, this->dataPtr->channels);
    break;
  case 32:
    ok = applyMultichannelLanczos3<float>(dst, _width, _height, src, w, h, this->dataPtr->channels);
    break;
  default:
    ok = false;
    gzerr << "Image: Unsupported number of bits per channel [" << this->dataPtr->bits_per_channel << "] \n";
  }
  stbi_image_free(this->dataPtr->bitmap);
  this->dataPtr->bitmap = NULL;
  if (ok) {
    this->dataPtr->bitmap = dst;
    this->dataPtr->width = _width;
    this->dataPtr->height = _height;
  } else {
    STBI_FREE(dst);
...
{
  Image::PixelFormatType types_8b[] = { UNKNOWN_PIXEL_FORMAT, L_INT8, UNKNOWN_PIXEL_FORMAT, RGB_INT8, RGBA_INT8 };
  Image::PixelFormatType types_16b[] = { UNKNOWN_PIXEL_FORMAT, L_INT16, UNKNOWN_PIXEL_FORMAT, RGB_INT16, RGBA_INT16 };
  Image::PixelFormatType types_32b[] = { UNKNOWN_PIXEL_FORMAT, R_FLOAT32, UNKNOWN_PIXEL_FORMAT, RGB_FLOAT32, UNKNOWN_PIXEL_FORMAT };
